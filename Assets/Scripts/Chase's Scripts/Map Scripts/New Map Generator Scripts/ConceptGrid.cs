 ...
 */
    public class ConceptGrid
    {
        ...
        /// <summary> Constructor for the concept grid class. </summary>
        ...
        public ConceptGrid(Vector2 gridDimension, NewGridInfo info)
        {
            ...
            int spawnRoomY = Random.Range(0, rows);

            grid[spawnRoomY, 0] = new NewGridNode(new Vector2(0, spawnRoomY), RoomType.Spawn);

            for (int y = 0; y < rows; y++)
            {
                if (y == spawnRoomY) continue;

                grid[y, 0].roomType = RoomType.Filled;
            }

            spawnRoomPos = new Vector2(0, spawnRoomY);
        ...
        private void GeneratePath()
        {
            ...
            foreach (List<NewGridNode> path in Paths)
            {
                for (int index = 1; index < path.Count; index++)
                {
                    ...
                    if (path[index - 1].roomType == RoomType.Spawn)
            ...
            void DetermineNumOpenings(NewGridNode currNode, NewGridNode prevNode, bool onlyOneOpening = false)
            {
                int numOpenings = 1;
        ...
        /// <returns></returns>
        private bool CheckSpawnBossDist(Vector2 spawnRoomPos, Vector2 bossRoomPos) =>
